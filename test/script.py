...
class NudeDetector:
    def __init__(self, providers=None):
        self.onnx_session = onnxruntime.InferenceSession(
            os.path.join("/content/best.onnx"),
    ...
    def censor(self, image_path, classes=[], output_path=None):
        ...
        img = cv2.imread(image_path)
        s_img = cv2.imread("/content/test.jpg", -1)
        count = 0
        for detection in detections:
            box = detection["box"]
            x, y, w, h = box[0], box[1], box[2], box[3]
            s_img_c = cv2.resize(s_img, (w,h))
            # change these pixels to your decided photo
            img[y : y + h, x : x + w,:] = s_img_c
            count+=1
...
ufile = fil
for fn in ufile.keys():
  path = fn
  os.rename(path, path.replace(' ', ''))
  fn = fn.replace(' ', '')
detectors.censor(fn, output_path="/content/2.jpg",)